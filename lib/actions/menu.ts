...
async function getMenuRestaurantIdFromSession(): Promise<string | null> {
  const branchId = getAuthActiveBranchId()
  if (!branchId) return null
  const supabase = createServerSupabase()
  return getMenuRestaurantIdForBranch(supabase, branchId)
}

export async function createProductAction(formData: FormData) {
  const restaurantId = await getMenuRestaurantIdFromSession()
