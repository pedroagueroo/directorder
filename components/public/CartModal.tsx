...
export default function CartModal({ restaurant, onClose }: { restaurant: Restaurant; onClose: () => void }) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      toast.success(
        paymentMethod === 'cash'
          ? `Pedido #${result.orderNumber} registrado. Ya está en cocina; el local marcará cuando cobre en efectivo.`
          : `Pedido #${result.orderNumber} registrado. El local confirmará el pago antes de mandarlo a cocina.`,
        { duration: 4500 }
      )
          ...
          <div className="space-y-4 pt-2 border-t border-border">
            ...
            >
              <button
                ...
                className={`min-h-11 touch-manipulation py-2.5 rounded-lg text-sm font-medium transition-colors active:scale-[0.98] ${!restaurant.delivery_enabled ? 'opacity-40 cursor-not-allowed' : ''} ${
              ...
              <button
                ...
                className={`min-h-11 touch-manipulation py-2.5 rounded-lg text-sm font-medium transition-colors active:scale-[0.98] ${!restaurant.pickup_enabled ? 'opacity-40 cursor-not-allowed' : ''} ${
            ...
            <div className="space-y-3">
              <div>
                <p className="text-xs font-semibold text-muted-foreground mb-2">Forma de pago</p>
                <div className="grid grid-cols-1 gap-1.5 p-1 bg-muted rounded-xl border border-border">
                  <button
                    type="button"
                    onClick={() => setPaymentMethod('other')}
                    className={`min-h-11 touch-manipulation py-2.5 px-3 rounded-lg text-sm font-medium text-left transition-colors ${
                      paymentMethod === 'other'
                        ? 'bg-card text-foreground shadow-sm'
                        : 'text-muted-foreground hover:text-foreground'
                    }`}
                  >
                    Transferencia / Mercado Pago u otro
                    <span className="block text-[11px] font-normal text-muted-foreground mt-0.5">
                      Cocina cuando el local confirme el pago
                    </span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    Efectivo
                    <span className="block text-[11px] font-normal text-muted-foreground mt-0.5">
                      Va directo a cocina; el cobro queda pendiente en el panel
                    </span>
                  </button>
                </div>
              </div>
        ...
        <div className="p-5 pb-safe border-t border-border bg-muted/20 sm:rounded-b-2xl shrink-0 space-y-3">
          <div className="space-y-1.5 px-0.5">
            ...
            <div className="flex justify-between items-baseline">
              ...
              <span className="text-sm font-semibold tabular-nums text-foreground">
                {deliveryFeeEstimate > 0 ? `$${deliveryFeeEstimate.toLocaleString('es-AR')}` : '$0'}
              </span>
            </div>
